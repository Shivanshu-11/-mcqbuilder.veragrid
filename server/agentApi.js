...
/**
 * Builds the /api/* request handler used by both the Vite dev server
 * (vite.config.js) and the production server (server/index.js).
 *
 * The handler runs the Python agent scripts that live in `pscadDir`.
 * Non-/api requests are passed through to `next()`.
 */
export function createAgentApiMiddleware({ pscadDir, pythonBin: pythonBinOverride } = {}) {
  const PSCAD_DIR = pscadDir;
  ...
  return async function agentApiMiddleware(req, res, next) {
